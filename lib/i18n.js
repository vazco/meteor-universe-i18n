...
export const i18n = {
    ...
    setLocale (locale, options = {}) {
        ...
        const {noDownload = false, silent = false, checkAvailability = true} = options;
        if (Meteor.isClient && !noDownload) {
            let promise = checkAvailability ? i18n.checkTranslationsAvailability(i18n._locale) : Promise.resolve();
            i18n._isLoaded[i18n._locale] = false;
            options.silent = true;
            if (i18n._locale.indexOf('-') !== -1) {
                promise = promise.then(() => i18n.loadLocale(i18n._locale.replace(/\-.*$/, ''), options))
                    .then(() => i18n.loadLocale(i18n._locale, options));
            } else {
                promise = promise.then(() => i18n.loadLocale(i18n._locale, options));
            }
            if (!silent) {
